...
//		                 {name:'rtnDt'		    	, index:'rtnDt'		    , sortable:true, width:100, align:"center"}
		                 {name:'rtnDtFmt'			, index:'RTN_DT'	   	, sortable:false		, width:100		,align:"center"		}
		            	,{name:'buySlipNo'	   		, index:'buySlipNo'   	, sortable:true, width:120, align:"center"}
		            	,{name:'buyNm'				, index:'buyNm'		    , sortable:true, width:200, align:"center"}
		            	,{name:'prdtCd'		    	, index:'prdtCd'		, sortable:true, width:100, align:"center"}
		            	,{name:'prdtNm'	  			, index:'prdtNm'		, sortable:true, width:250, align:"left"}
		            	,{name:'prdtStd'	    	, index:'prdtStd'	    , width:100, align:"center"}
		            	...
		            	,{name:'remarks'			, index:'remarks'	    , width:300, align:"left"}
		            	,{name:'regId'		        , index:'regId'	     	, sortable:true, width:100, align:"center"}
		           		,{name:'regDt'			    , index:'regDt'		    , sortable:true, width:200, align:"center"}
...
// 	            }
	...
	function searchEvent(event){
		...
		$("#tabList").jqGrid('setGridParam',{
			url:"<c:url value='/app/br/buy/buyRtnList_2_SelList.json'/>",
			datatype: "json",
			postData: searchInfo,
...
//			$('#prdtNm').focus();
	...
	function findPrdtMst() {
		commonPrdtMstFindLayer('','',$("#prdtNm").val(),  $("#srchBuyCd").val(), setPrdtMstFindLayer);
	}
	
	/*상품 콜백(후처리 함수 )*/
	function setPrdtMstFindLayer(data) {
		if (data != null){
			$("#srchPrdt").val(data.prdtCd);
	...
	/* 엑셀  */
	function excelEvent(){
		$('#searchForm').attr("action", "<c:url value='/app/br/buy/buyRtnList_2_SelExcelDwon'/>").submit();	
	}
		...
		<tbody id="_search">
			<tr>
				<th><label for="sele2">반품일자</label></th>
					<td>
						<input type="text" class="dt" id="srchFrDt" 	name="srchFrDt"  	style="width: 15% !important;" maxlength="8" value="${util:getYearMonthFirstDay()}"  />
						<input type="text" class="dt" id="srchToDt" 	name="srchToDt"  	style="width: 15% !important;" value="<fmt:formatDate value="${now}" pattern="${localeDatePattern}" />"/> 
					</td>
				<th><label for="sele2">반품구분</label></th>
					<td>
						<html:codeTag comType="SELECT" objId="srchRtnClass" objName="srchRtnClass" parentID="M012"  defName="${commonall}"   />
					</td>
			</tr>
			<tr>
				<th><label for="sele2">매입처명</label></th>
					<td>				
						...
						<input type="hidden" id="srchBuyCd"  name="srchBuyCd" >
						<button id="btnBuyCd"  class="button btn_find" type="button"><i class="fa fa-search"></i></button>
				</td>
				<th><label for="sele2">품목명</label></th>
					<td>
						<input type="text"   id="prdtNm"  	name="prdtNm" style="width:40%;" onclick="prdt_clear()">
						<input type="hidden" id="srchPrdt"  name="srchPrdt" >
