...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('#btnBuyInpectPrt').unbind().click(null,	prtEvent);
		
		$('#btnBuyCd').unbind().click(function(){ findBuyMst()}); 	// 매출처찾기 팝업 버튼이벤트
	...
	function initBuyInspectGrid() {
		$("#tabList").jqGrid({
			...
			,colModel:[
		                 {name:'buyNm'			, index:'buyNm'			, sortable:true, width:160, align:"left"}
		            	...
		            	,{name:'ordUnit'		, index:'ordUnit'		, sortable:true, width:100, align:"center"}
		            	,{name:'timeLimit'		, index:'timeLimit'		, sortable:true, width:120, align:"center"}
		            	,{name:'termVal'		, index:'termVal'		, sortable:true, width:120, align:"center"}
		            	,{name:'bIssQty'		, index:'bIssQty'		, sortable:true, sorttype:'number', width:100, align:"right", formatter:'currency', formatoptions:{thousandsSeparator:",",decimalPlaces: 2}}
                    	,{name:'wIssQty'		, index:'wIssQty'		, sortable:true, sorttype:'number', width:100, align:"right", formatter:'currency', formatoptions:{thousandsSeparator:",",decimalPlaces: 2}}
		            	...
		            	,{name:'ordDt'			, index:'ordDt'			, sortable:true, width:120, align:"center"}
		            	
		            	,{name:'stdYn'			, index:'stdYn'			, sortable:true, hidden:true, width:120, align:"center"}
		            	,{name:'qtyBox'			, index:'qtyBox'		, sortable:true, hidden:true, width:120, align:"center"}
		  	...
		  	gridComplete : function() {                                      //데이터를 성공적으로 가져오면 실행 됨
				...
				// ===================== Footer Sum
				gridData.jqGrid('footerData', 'set', { 'ordUnit' : '계' });
		
		    	var sum_bIssQty = gridData.jqGrid('getCol','bIssQty', false, 'sum');
		    	gridData.jqGrid('footerData', 'set', { 'bIssQty':sum_bIssQty });
		    	
		    	var sum_wIssQty = gridData.jqGrid('getCol','wIssQty', false, 'sum');
		    	gridData.jqGrid('footerData', 'set', { 'wIssQty':sum_wIssQty });
	...
	function searchEvent(event){
		...
		$("#tabList").jqGrid('setGridParam',{
			url:"<c:url value='/app/br/buy/buyInspectList_selList.json'/>",
			datatype: "json",
			postData: searchInfo,
	...
	function prtEvent(){
		...
		var url = "<c:url value="/app/br/buy/buyInspectListPrint" />";
		var target = "PDFPrint";
	...
	/* 엑셀  */
	function excelEvent(){
		$('#searchForm').attr("action", "<c:url value='/app/br/buy/purBuyInspectList_selExcelDown'/>").submit();
	}
...
<sec:csrfInput/>
	<fieldset>
	<legend>매입검수확인서</legend>
	<table style="width: 100%" summary="" class=type1>
		<caption>매입검수확인서 검색조건</caption>
		<colgroup>
			<col width="100">
			<col width="*">
			<col width="100">
		...
		<tbody id="_search">
			<tr>
				<th><label for="sele2">일자</label></th>
					<td>
						<input type="text" class="dt" id="buyStartDt" name="buyStartDt"  style="width: 25% !important;" value="<fmt:formatDate value="${now}" pattern="${localeDatePattern}" />"/>
						<input type="text" class="dt" id="buyEndDt"   name="buyEndDt"    style="width: 25% !important;" value="<fmt:formatDate value="${now}" pattern="${localeDatePattern}" />"/> 
					</td>
					   <th><label for="sele2">입고구분</label></th>
					<td>
						<html:codeTag comType="SELECT"  objId="searchUseYn"  objName="useYn" parentID="M009" defName="${commonall}" /> <%--전체 --%>
					</td>
			...
			<tr>
				...
				<th><label for="sele2">중분류</label></th>
					<td>
						<html:codeTag comType="SELECT" 	dataType="PRC2"	objId="mCd" objName="mCd" parentID="Y"  width="80%" defName="${commonall}" />
					</td>
			</tr>
		</tbody>
	</table>
 	...
 	<form name="mainForm" id="mainForm"  >
 		...
 		<div class="tit_area">
			...
			<div class="btn_l">
				...
				<html:button id="btnBuyInpectPrt" 		name="btnBuyInpectPrt"   		auth="select"  value="출력"  />
			</div>
		</div>
