		...
		$("#tabList").jqGrid({
			...
			gridComplete : function() {                                      //데이터를 성공적으로 가져오면 실행 됨
				...
				if(allRows.length > 0 && $("#blankRow").val() != ""){
					...
					gridData.jqGrid('setRowData', allRows[0], false, {color:'red'});
					gridData.jqGrid('setRowData', allRows[1], false, {color:'red'});
					gridData.jqGrid('setRowData', allRows[2], false, {color:'red'});
...
//			    $('table.ui-jqgrid-ftable tr:eq(0)').css("color","red");			//footer row font color
	...
	function searchEvent(event){
		...
		$("#tabList").jqGrid('setGridParam',{
			url:"<c:url value='/app/as/sal/salSalesYearOnYearTotList_selList.json'/>",
			datatype: "json",
			postData: searchInfo,
...
<form id="searchForm" name="searchForm" method="post" autocomplete="off">
	<sec:csrfInput/>
	<fieldset>
	<legend>전년대비매출처별매출현황(합계)</legend>
	<table style="width: 100%" summary="" class=type1>
		<caption>전년대비매출처별매출현황(합계) 검색조건</caption>
		<colgroup>
			<col width="100">
			<col width="*">
			<col width="100">
...
<!-- 추가저장정보 end ----------------------------------------- -->
 ...
 <UL style="width: 100%;">
 	...
 	<form name="mainForm" id="mainForm"  >
 		<div class="tit_area">
			<h2 class="subhead">전년대비매출처별매출현황(합계)</h2>
			<div class="btn_l">
