...
$(document).ready(function(){
	...
	initAccSalesDepRegGrid();
	initAccSalesDepRegDetailGrid();
...
/* 마스터 데이터  그리드 초기화 */
function initAccSalesDepRegGrid() {
	$("#container1List").jqGrid({
		...
		,colModel:[
	                 {name:'depDt'			, index:'depDt'			, sortable:true, width:50 , align:"center" }
	                ,{name:'salesCd'		, index:'salesCd'		, sortable:true, width:30 , align:"center"}
	                ,{name:'salesNm'		, index:'salesNm'		, sortable:true, width:100 , align:"left"}
...
/* 세부코드 그리드 초기화 */
function initAccSalesDepRegDetailGrid() {
    $("#container2List").jqGrid({
        ...
        ,colModel:[
             {name:'depDt'    	  , index:'depDt'		, sortable:false  , width:80    , align:"center"}
        	...
        	,{name:'regDt' 		  , index:'regDt'		, sortable:false  , width:110 	, align:"center"}
     		...
     		,{name:'salesCd' 	, index:'salesCd'		, sortable:false  , width:150 	, align:"left", hidden:true}
...
function findSalesMst(){
	commonSalesMstFindLayer('','',$("#salesNm").val(), 'Y', 'Y', setSalesyMstFindLayer);
}
...
function setSalesyMstFindLayer(data) {
	...
	if(data != null && data.salesCd) $("#salesCd").val(data.salesCd);		/*매출처코드*/
	if(data != null && data.salesNm)  $("#salesNm").val(data.salesNm);		/*매출처명*/
...
function searchEvent(event){
	...
	$("#container1List").jqGrid('setGridParam',{
		url:"<c:url value='/app/as/account/accSalesDepReg_selList.json'/>",
		datatype: "json",
		postData: searchInfo,
...
function cusShipRegDetail(rowObj){
	...
	var str = rowObj;
  	...
  	$("#container2List").jqGrid('setGridParam',{
        url:'<c:url value="/app/as/account/accSalesDepReg_selDetailList.json" />', 		
        datatype: "json",     
        postData:str,         
        mtype:'POST',         
        loadBeforeSend: function(jqXHR) {
...
function saveEvent(){
	...
	var saveData = {'accSalesDepRegList'  : $("#container2List").getRowData()
	               };

	$.ajax({
	      contentType : 'application/json; charset=utf-8', type : 'post', dataType : 'json',
	      url  : '<c:url value="/app/as/account/accSalesDepReg_insInfo.json" />',
...
<fieldset>
	<legend>매출처입금등록</legend>
	<table style="width: 100%" summary="" class=type1>
		<caption>매출처입금등록 검색조건</caption>
		<colgroup>
			<col width="100">
			<col width="*">
			<col width="100">
		...
		<tbody id="_search">
			<tr>
				<th><label for="sele2">입금일자</label></th>
					<td>
						<input type="text" class="dt" id="depDt" name="depDt" style="width: 40% !important;" maxlength="8" value="<fmt:formatDate value="${now}" pattern="${localeDatePattern}" />"  />
					</td>
				<th><label for="sele2">매출처명</label></th>
					<td>
						<input type="text" id="salesNm" name="salesNm" style="width: 80%;" onclick="sales_clear()" autocomplete="off">
