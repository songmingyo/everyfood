...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('#btnPrint').unbind().click(null,	        prtEvent);    // 출력
	...
	function initDspListGrid() {
		$("#tabList").jqGrid({
			...
			colModel:[
						{name:'mvDt'			, index:'mvDt'			, sortable:true, width:100 , align:"center"}
						,{name:'outWhCd'		, index:'outWhCd'		, sortable:true, width:100 , align:"center"}
						,{name:'outWhNm'		, index:'outWhNm'		, sortable:true, width:100 , align:"center"}
						,{name:'inWhCd'			, index:'inWhCd'		, sortable:true, width:100 , align:"center"}
						,{name:'inWhNm'			, index:'inWhNm'		, sortable:true, width:100 , align:"center"}
		            	...
		            	,{name:'ordUnitNm'		, index:'ordUnitNm'		, sortable:true, width:150 , align:"center"}
		            	,{name:'mvQty'			, index:'mvQty'			, sortable:true, width:100 , align:"right", formatter:'integer'} 
	...
	function searchEvent(event){
		...
		$("#tabList").jqGrid('setGridParam',{
			url:"<c:url value='/app/cs/logWhMvList_selList.json'/>",
			datatype: "json",
			postData: searchInfo,
	...
	/* 엑셀  */
	function excelEvent(){
		$('#searchForm').attr("action", "<c:url value='/app/cs/logWhMvList_selExcelDown'/>").submit();
	}
	
	/*센터이동집계표*/
	function prtEvent(){
		
		var url = "<c:url value="/app/cs/logWhMvList_prtList" />";
		var target = "PDFPrint";
...
<form id="searchForm" name="searchForm" method="post" autocomplete="off">
	<sec:csrfInput/>
	<fieldset>
	<legend>센터 이동현황</legend>
	<table style="width: 100%" summary="" class=type1>
		<caption>센터 이동현황 검색조건</caption>
		<colgroup>
			<col width="100">
			<col width="*">
			<col width="100">
 	...
 	<form name="mainForm" id="mainForm"  >
 		<div class="tit_area">
			...
			<div class="btn_l">
				...
				<html:button id="btnPrint" 			name="btnPrint"   		auth="print" value="센터이동집계표" /> 		<%-- 출력 --%>
			</div>
		</div>
