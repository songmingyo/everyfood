...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#startDt").datepicker();
		$("#endDt").datepicker();
		initCusShipAgrclListGrid();

		/* BUTTON CLICK 이벤트 처리 ------------------------------------------*/
		$('#btnSearch').unbind().click(null,	    searchEvent); 	// 검색
		$('#btnNew').unbind().click(null,	        newEvent);    // 신규 
		$('#btnExcel').unbind().click(null,	    	excelEvent);	// 액샐 
		$('#btnAgrcPrt').unbind().click(null,		agrcPrt); 	 // 출력
		
		$('#btnPrdt_find').unbind().click(function(){ findPrdtMst()}); 	// 상품찾기 팝업 버튼이벤트
		$('#btnPrdt_clear').unbind().click(function(){ setPrdtMstFindLayer({find_prdtCd:'',  find_prdtNm:''})}); 	// 상품찾기 초기화 버튼이벤트		
	...
	/* 마스터 데이터  그리드 초기화 */
	function initCusShipAgrclListGrid() {
		$("#tabList").jqGrid({
			...
			,colModel:[
		                 {name:'dt'		    	    , index:'dt'		    , sortable:true, width:120 , align:"center"}
		            	...
		            	,{name:'saveKind'		    , index:'saveKind'		, sortable:true, width:100, align:"center"}
		            	,{name:'carNm'		    	, index:'carNm'		    , sortable:true, width:140, align:"center"}
		            	...
		            	,{name:'ordUnitNm'		    , index:'ordUnitNm'		, sortable:true, width:100, align:"center"}
		            	,{name:'qty'		        , index:'qty'		    , sortable:true, sorttype:'number', width:110, align:"right", formatter:'currency', formatoptions:{thousandsSeparator:",",decimalPlaces: 2}}
		            	,{name:'gubun'		        , index:'gubun'	     	, sortable:true, width:110, align:"center"}
		     ],
			gridComplete : function() {                                      //데이터를 성공적으로 가져오면 실행 됨
				...
				// ===================== Footer Sum
				gridData.jqGrid('footerData', 'set', { 'dt' : '합계' });
	...
	function searchEvent(event){
		...
		$("#tabList").jqGrid('setGridParam',{
			url:"<c:url value='/app/sm/custsales/cusShipAgrclList_selList.json'/>",
			datatype: "json",
			postData: searchInfo,
	...
	function agrcPrt(){
		...
		searchForm.action="<c:url value="/app/sm/custsales/cusShipAgrcListPrint" />";
		searchForm.submit();
	}
	
	/* 엑셀  */
	function excelEvent(){
		$('#searchForm').attr("action", "<c:url value='/app/sm/custsales/cusShipAgrclList_selExcelDown'/>").submit();
	}
...
<sec:csrfInput/>
	<fieldset>
	<legend>매출처출고현황(농산물)</legend>
	<table style="width: 100%" summary="" class=type1>
		<caption>매출처출고현황(농산물) 검색조건</caption>
		<colgroup>
			<col width="150">
			<col width="*">
			<col width="150">
		...
		<tbody id="_search">
			<tr>
				...
				<th><label for="sele2">매출처명</label></th>
					<td>
						<c:choose>
							<c:when test ="${userSession.userType eq 'S2'}">
							...
							<c:otherwise>
								<input type="text" id="salesNm" name="salesNm" style="width: 80%;" readOnly>
...
<%-- 				<html:button id="btnAgrcPrt" 		name="btnAgrcPrt"   		auth="select"  value="출력"  /> --%>
        	</div>
		</div>
		<!-- 서브타이틀 및 이벤트 버튼 end  -------- -->
