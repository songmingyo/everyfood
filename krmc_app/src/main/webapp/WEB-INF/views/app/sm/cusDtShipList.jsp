...
<script type="text/javascript">
	...
	/* 마스터 데이터  그리드 초기화 */
	function initCusDtShipListGrid() {
		$("#tabList").jqGrid({
			...
			,colModel:[
		            	...
		            	,{name:'salesDt'	      	, index:'salesDt'	, sortable:true, width:120, align:"center"}
		            	,{name:'whNm'				, index:'whNm'		, sortable:true, width:160, align:"center"}
		            	,{name:'pureAmt'		    , index:'pureAmt'	, sortable:true, sorttype:'number', width:150, align:"right", formatter:'integer'}
		            	,{name:'vatAmt'		    	, index:'vatAmt'	, sortable:true, sorttype:'number', width:150, align:"right", formatter:'integer'}
		            	,{name:'totAmt'	  			, index:'totAmt'	, sortable:true, sorttype:'number', width:180, align:"right", formatter:'integer'}
			...
			,gridComplete : function() {                                      //데이터를 성공적으로 가져오면 실행 됨
				...
				// ===================== Footer Sum
				gridData.jqGrid('footerData', 'set', { 'salesDt' : '합계' });
...
// 	        	$(window).resize();
	...
	function searchEvent(event){
		...
		$("#tabList").jqGrid('setGridParam',{
			url:"<c:url value='/app/sm/custsales/cusDtShipList_selList.json'/>",
			datatype: "json",
			postData: searchInfo,
	...
	/* 엑셀  */
	function excelEvent(){
		$('#searchForm').attr("action", "<c:url value='/app/sm/custsales/cusDtShipList_selExcelDown'/>").submit();
	}
...
<sec:csrfInput/>
	<fieldset>
	<legend>매츨처일자별출고현황</legend>
	<table style="width: 100%" summary="" class=type1>
		<caption>매츨처일자별출고현황 검색조건</caption>
		<colgroup>
			<col width="100">
			<col width="*">
			<col width="100">
