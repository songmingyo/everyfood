...
function fncInitSearch(){
	...
	$("#tabList").jqGrid({
		...
		colModel:[
			...
			{name:'accessDyFmt'		, index:'ACCESS_DY'			, sortable:false		, width:150		,align:"center"		},
			{name:'persBizsCdNm'	, index:'PERS_BIZS_CD_NM'	, sortable:false		, width:180		,align:"left"		},
			{name:'persPclInfo'		, index:'PERS_PCL_INFO'		, sortable:false		, width:180		,align:"center"		},
			{name:'persInfo'		, index:'PERS_INFO'			, sortable:false		, width:180		,align:"left"		},
			{name:'rendDivnCdNm'	, index:'REND_DIVN_CD_NM'	, sortable:false		, width:350		,align:"left"		},
			{name:'userNm'			, index:'USER_NM'			, sortable:false		, width:100		,align:"center" 	},
			{name:'regDateFmt'		, index:'REG_DT'			, sortable:false		, width:200		,align:"center"		},
			{name:'rendDivnCd'		, index:'REND_DIVN_CD'		, hidden:true},
			{name:'persBizsCd'		, index:'PERS_BIZS_CD'		, hidden:true},
		],
...
function eventSearch(){
	...
	$("#tabList").jqGrid('setGridParam',{
		url:'<c:url value="/app/mgr/manager/mgrUserInfoCheckHist_selectUserCheckList.json"/>',		// url 주소
...
function fncExcelDown(){
	...
	var srchPersBizsCd = $("#searchForm #srchPersBizsCd").val();
	var srchRendDivnCd = $("#searchForm #srchRendDivnCd").val();
	var srchFrDy = $("#searchForm #srchFrDy").val().replaceAll("-","");
	var srchToDy = $("#searchForm #srchToDy").val().replaceAll("-","");
	
	$('#excelForm input[name="srchPersBizsCd"]').val(srchPersBizsCd);
	$('#excelForm input[name="srchRendDivnCd"]').val(srchRendDivnCd);
	$('#excelForm input[name="srchFrDy"]').val(srchFrDy);
	$('#excelForm input[name="srchToDy"]').val(srchToDy);
	
	$('#excelForm').attr("action", "<c:url value='/app/mgr/manager/mgrUserInfoCheckHist_selectUserCheckListExcelDown'/>").submit();
}		
...
<form id="excelForm" name="excelForm" method="post">
	<sec:csrfInput/>
	<input type="hidden" name="srchPersBizsCd" />
	<input type="hidden" name="srchRendDivnCd" />
...
<div id="section">
	...
	<form id="searchForm" name="searchForm" method="post">
			...
			<table class="type1 inputWidth">
				...
				<tbody>
					<tr>
						...
						<th>열람사유</th>
						<td>
							<html:codeTag comType="SELECT" objName="srchRendDivnCd" parentID="9016" objId="srchRendDivnCd"  defName="${commonall}"/>
						</td>
						<th>업무구분</th>
						<td>
							<html:codeTag comType="SELECT" objName="srchPersBizsCd" parentID="9017" objId="srchPersBizsCd"  defName="${commonall}"/>
						</td>
					</tr>
				</tbody>
			</table>
		...
		<!-- 분류 서브타이틀 및 이벤트 버튼 -->
		<div class="tit_area">
			<h2 class="subhead">개인정보 조회이력 List</h2>
			<div class="btn_l">
