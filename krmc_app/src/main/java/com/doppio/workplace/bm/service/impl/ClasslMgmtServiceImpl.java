...
/**
 * @author j10000
 * @Description :대분류코드관리 ClasslMgmtService implement
 * @Class : ClasslMgmtServiceImpl
 * <pre>
...
@Service("classlMgmtService")
public class ClasslMgmtServiceImpl  implements ClasslMgmtService{
	
	@Autowired
	ClasslMgmtMapper classlMgmtMapper;
	 ...
	 * @throws Exception
	 */
	public HashMap<String, Object> selectClasslMgmtList(ClasslMgmtVo paramVo) throws Exception {
		
	
		
		HashMap<String, Object> returnMap = new HashMap<String,Object>();
		List<ClasslMgmtVo> resultList = null;
		...
		int totalCount = 0;
		totalCount = classlMgmtMapper.selectClasslMgmtListCount(paramVo);
		...
		if(totalCount>0) {
			resultList =  classlMgmtMapper.selectClasslMgmtList(paramVo);
		}
	 ...
	 * @throws Exception
	 */
	public ClasslMgmtVo selectClasslMgmtData(ClasslMgmtVo paramVo) throws Exception {
		return  classlMgmtMapper.selectClasslMgmtData(paramVo);
	}
	...
	public Result insertClasslMgmt(HttpServletRequest request, ClasslMgmtVo paramVo) throws Exception{
		...
		int executeCnt = 0;
		if(StringUtil.isNotEmpty(paramVo.getlCd()) ) 
			 executeCnt = classlMgmtMapper.updateClasslMgmt(paramVo);		// 수정 
		else executeCnt = classlMgmtMapper.insertClasslMgmt(paramVo);		// 신규 생성 ( BEFOR select CAR_CD ) 

		if(executeCnt == 1) {
			result.setMsgCd("success");
			result.setRtnValue01(paramVo.getlCd());	// 수정,등록 CAR_CD 
		}
	 ...
	 * @throws Exception
	 */
	public Result deleteClasslMgmtData(ClasslMgmtVo paramVo) throws Exception {
		
		Result result = new Result();
		result.setMsgCd("fall");
		
		int executeCnt = 0;
		 executeCnt = classlMgmtMapper.deleteClasslMgmt(paramVo);
