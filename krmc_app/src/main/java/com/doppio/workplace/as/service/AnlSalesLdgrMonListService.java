package com.doppio.workplace.as.service;

import java.util.HashMap;
import java.util.List;

/**
 *
 * @Class : AnlSalesLdgrMonListService.java
 * @Description : 매출처원장(월별)
 * @author : j10000
 * <pre>
 *  << 개정이력(Modification Information) >>
 *
 *          수정일          수정자           수정내용
 *  ----------------    ------------    ---------------------------
 *   2023. 5. 18.      j10000        	  
 *
 * </pre>
 */

public interface AnlSalesLdgrMonListService {
	
	
	

	/**
	 * 매출처원장(월별) 조회
	 * @param AnlSalesLdgrMonListVo	paramVo
	 * @return List<AnlSalesLdgrMonListVo>
	 * @throws Exception
	 */
	HashMap<String, Object> selectAnlSalesLdgrMonList(AnlSalesLdgrMonListVo paramVo) throws Exception;
	


	/**
	 * 매출처원장(월별) 엑셀 다운로드
	 * @param paramVo
	 * @return
	 * @throws Exception
	 */
	public List<HashMap<String, String>> selectAnlSalesLdgrMonListExcelDown(AnlSalesLdgrMonListVo paramVo) throws Exception;


}
